...
- **Authentication**: Clerk (with Korean localization)
  - 한국어 로컬라이제이션 적용 (`lib/clerk/localization.ts`)
  - 커스텀 메시지 및 에러 메시지 커스터마이징 가능
  - 참고: [Clerk 로컬라이제이션 가이드](docs/CLERK_LOCALIZATION.md)
...
2. **Supabase 클라이언트 파일들** (`lib/supabase/`):
   ...
   - `clerk-client.ts`: Client Component용 (useClerkSupabaseClient hook)
     ...
     - `accessToken()` 함수로 Clerk 토큰 자동 전달
...
4. **예제**:
   - `app/tasks-example/page.tsx`: 완전한 CRUD 예제 페이지
   - `supabase/migrations/20250101000000_create_tasks_table.sql`: 예제 테이블 스키마
