...
- **한국어 UI 지원** (Clerk 컴포넌트 완전 한국어화)
  - 커스텀 로컬라이제이션 파일로 관리 (`lib/clerk/localization.ts`)
  - 에러 메시지 커스터마이징 가능

### 🗄️ 데이터베이스 통합
- Clerk 토큰 기반 Supabase 인증 (JWT 템플릿 불필요)
- **Supabase 공식 문서의 모범 사례 준수**:
  - `@supabase/ssr` 패키지 사용
  - Server Component: `createServerClient` (Next.js 15 `cookies()` API 활용)
  - Client Component: `createBrowserClient` (자동 쿠키 관리)
...
**📚 상세 가이드**: [docs/CLERK_SUPABASE_INTEGRATION.md](docs/CLERK_SUPABASE_INTEGRATION.md) 참고

**3-1. Clerk 대시보드에서 Supabase 통합 활성화**
...
- `/tasks-example`: 완전한 CRUD 예제 (Client Component)
- `/tasks-example-server`: 완전한 CRUD 예제 (Server Component, Supabase 공식 모범 사례)
...
**설정 위치:**
- `app/layout.tsx`: `ClerkProvider`에 `customKoKR` 로컬라이제이션 적용
- `lib/clerk/localization.ts`: 커스텀 한국어 로컬라이제이션 파일

**커스터마이징:**
- `lib/clerk/localization.ts` 파일에서 특정 텍스트나 에러 메시지를 수정할 수 있습니다
- 자세한 내용은 [Clerk 로컬라이제이션 가이드](docs/CLERK_LOCALIZATION.md)를 참고하세요

**참고:**
- Clerk 로컬라이제이션은 현재 실험적(experimental) 기능입니다
...
### 프로젝트 문서
- [Clerk + Supabase 통합 가이드](docs/CLERK_SUPABASE_INTEGRATION.md) - 상세한 통합 가이드 및 예제
- [Clerk 한국어 로컬라이제이션 가이드](docs/CLERK_LOCALIZATION.md) - 한국어 설정 및 커스터마이징
- [AGENTS.md](AGENTS.md) - 개발 가이드 및 컨벤션
